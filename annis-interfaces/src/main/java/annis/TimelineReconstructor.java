...
public class TimelineReconstructor {
   ...
   * @param spanAnno2order A map from all span annotation names to the named order relation they
   *        belong to.
   */
  ...
  private void createTokenFromSOrder() {
    ...
    // also add nodes that are are marked as start by ANNIS even if they don't have an outgoing
    // order rel
    for (SSpan n : graph.getSpans()) {
      SFeature feat =
          n.getFeature(AnnisConstants.ANNIS_NS, AnnisConstants.FEAT_FIRST_NODE_SEGMENTATION_CHAIN);
