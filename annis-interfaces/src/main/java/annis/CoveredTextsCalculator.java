...
/**
 * Traverses the Salt graph and gets the covered {@link STextualDS} for a list of nodes.
 * 
...
public class CoveredTextsCalculator implements GraphTraverseHandler {
  ...
  public CoveredTextsCalculator(SDocumentGraph graph, List<SNode> startNodes) {
    texts = new LinkedHashSet<STextualDS>();
    if (startNodes.size() > 0) {
      graph.traverse(new ArrayList<SNode>(startNodes), GRAPH_TRAVERSE_TYPE.TOP_DOWN_DEPTH_FIRST,
          "CoveredTextsCalculator", this, true);
    }
  }
  ...
  public boolean checkConstraint(GRAPH_TRAVERSE_TYPE traversalType, String traversalId,
    ...
    if (relation == null || relation instanceof SDominanceRelation
        || relation instanceof SSpanningRelation || relation instanceof STextualRelation) {
      return true;
    } else {
      return false;
    }
