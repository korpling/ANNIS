...
public class HTMLVis extends AbstractVisualizer<Panel> {
    ...
    public VisualizationDefinition[] parseDefinitions(String toplevelCorpusName, Properties mappings, UI ui) {
        ...
        if (visConfigName == null) {
            ...
            WebResource resBinary = Helper.getAnnisWebResource(ui).path("query/corpora/").path(toplevelCorpusName)
        ...
        if (inStreamConfigRaw == null) {
            Notification.show("ERROR: html visualization configuration \"" + visConfigName + "\" not found in database",
    ...
    private void injectWebFonts(String visConfigName, String corpusName, UI ui) {
        ...
        if (inStreamJSONRaw != null) {
            try (InputStream inStreamJSON = inStreamJSONRaw) {
                ...
                Notification.show("Could not parse the HTML visualizer web-font configuration file", ex.getMessage(),
                        Notification.Type.ERROR_MESSAGE);
            }
        }
    }
    ...
    public String createHTML(SDocumentGraph graph, VisualizationDefinition[] definitions, UI ui) {
        ...
        for (VisualizationDefinition vis : definitions) {
            ...
            if (vis.getMatcher() instanceof PseudoRegionMatcher) {
                PseudoRegionMatcher.PseudoRegion psdRegionType = ((PseudoRegionMatcher) vis.getMatcher())
                        .getPsdRegion();
                int positionStart = 0;
                int positionEnd = 0;
                ...
                case META_NAME:
                    ...
                    if (strMetaVal == null) {
                        throw new NullPointerException(
                                "no such metadata name in document: '" + vis.getOutputter().getMetaname().trim() + "'");
                    } else {
