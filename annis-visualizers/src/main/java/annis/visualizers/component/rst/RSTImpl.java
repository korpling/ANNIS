...
public class RSTImpl extends Panel implements GraphTraverseHandler {
	 ...
	 * mapping the sentence to a number by the order of the SStructures in the list.
	 */
	...
	public RSTImpl(VisualizerInput visInput) {
		...
		this.sentences = new TreeSet<SStructure>(new Comparator<SStructure>() {
			...
			public int compare(SStructure t1, SStructure t2) {
				...
				int byStart = Integer.compare(getStartPosition(overlappedT1), getStartPosition(overlappedT2));
				if (byStart == 0) {
	...
	@Override
	public boolean checkConstraint(GRAPH_TRAVERSE_TYPE traversalType, String traversalId, SRelation incomingEdge,
	...
	/**
	 * Gets the overlapping token as string from a node, which are direct dominated
	 * by this node.
	 *
	 * @param currNode
	...
	private String getText(SToken currNode) {
		 ...
		 * chapter "5.8 More specific nodes and relations" the start and end point of a
		 * range of token is stored in superordinate node of type SSequentialDS
		 */
	...
	private String getHTMLColor(SToken token) {
		...
		/**
		 * Since the range in markedAndCovered is from 1 up to 8, we have to decrease
		 * the value, for matching the colors in KWIC.
		 */
