...
@JavaScript({ "SentStructureJs_Connector.js", 
	"vaadin://jquery.js", 
	"SentStructure.js", 
	"d3.min.js" })

/**
 * {@link SentStructureJsComponent} visualizes Salt model of a specified
 * [VisualizerInput](\ref annis.libgui.visualizers.VisualizerInput).
 * 
 * @author Adriane Boyd
 *
 */
...
public class SentStructureJsComponent extends AbstractJavaScriptComponent implements ExportFilter, StyleImporter {
	...
	public enum Annos_Keyword {
		NODE_ANNOS_KW(annis.visualizers.component.grid.GridComponent.MAPPING_ANNOS_KEY), POINTING_REL_ANNOS_KW(
	 ...
	 * 
	 * @param visInput
	 *            the specified [VisualizerInput](\ref
	 *            annis.libgui.visualizers.VisualizerInput)
	 */
		...
		for (SLayer layer : tokensByLayer.keySet()) {
			...
			for (SToken token : lTokens) {
				...
				if (visInput.getMarkedAndCovered().containsKey(token)) {
					tokenColor = MatchedNodeColors.getHTMLColorByMatch(visInput.getMarkedAndCovered().get(token));
				}
	 ...
	 *
	 * This will check the "relation" parameter for determining the annotations to
	 * display. It also iterates over all nodes of the graph matching the type.
	 *
	 * @param input
	 *            The input for the visualizer.
	 * @param type
	 *            Which type of relations to include
	 * @return
	 */
	private static List<String> computeDisplayedRelAnnotations(VisualizerInput input, String relAnnosConfiguration,
	 ...
	 *            The graph.
	 * @param namespace
	 *            The namespace of the relation (not the annotation) to search for.
	 *            If namespace is null all namespaces will be considered.
	 * @param type
	 *            Which type of relation to include
	 * @return
	 *
	 */
	...
	/**
	 * Implements the includeNode method of the
	 * org.corpus_tools.salt.util.ExportFilter interface.
	 */
	@Override
	...
	/**
	 * Implements the includeRelation method of the
	 * org.corpus_tools.salt.util.ExportFilter interface.
	 */
	...
	/**
	 * Implements the getHighlightingColor method of the
	 * org.corpus_tools.salt.util.StyleImporter interface.
	 */
	@Override
	...
	private String getPointingRelationAnnotation(SPointingRelation rel) {
		...
		if (mappings.containsKey(MAPPING_ALIGNMENT_LABEL) && mappings.getProperty(MAPPING_ALIGNMENT_LABEL) != null) {
