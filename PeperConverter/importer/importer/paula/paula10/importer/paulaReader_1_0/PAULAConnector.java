...
import importer.paula.paula10.importer.paulaReader_1_0.reader.CorpStructDataReader;
import importer.paula.paula10.importer.paulaReader_1_0.reader.PAULAReader;
import importer.paula.paula10.importer.paulaReader_1_0.util.PAULAReaderMgr;
import importer.paula.paula10.importer.paulaReader_1_0.util.TypeFileReader;
...
//	 ============================================== öffentliche Methoden ==============================================
	...
	private class PAULADocFeatures
	{
		public String cType= null;
		public String paulaFile= null;
	}
	
	private Map<String, Collection<PAULADocFeatures>> doc2pauladocTable= new Hashtable<String, Collection<PAULADocFeatures>>();; 
	/**
									...
									String corpusPath) throws Exception
	{
		...
		//if no entry slot exists create one
		if (!this.doc2pauladocTable.containsKey(corpusPath))
		{
			Collection<PAULADocFeatures> slot= new Vector<PAULADocFeatures>();
			this.doc2pauladocTable.put(corpusPath, slot);
		}
		
		//put a new pauladocFeature into slot
		PAULADocFeatures paulaDocFeature= new PAULADocFeatures();
		paulaDocFeature.cType= cType;
		paulaDocFeature.paulaFile= paulaFile;
		this.doc2pauladocTable.get(corpusPath).add(paulaDocFeature);
...
//		pReader.parse(pFile);
	...
	public void readDocumentContent(String corpusPath) throws Exception
	{
		for (PAULADocFeatures paulaDocFeature: this.doc2pauladocTable.get(corpusPath))
		{
			File pFile= new File(paulaDocFeature.paulaFile);
			PAULAReader pReader= this.paulaReaderMgr.getPAULAReader(paulaDocFeature.cType, pFile, corpusPath, this.mapper);
			//prüfe ob ein Reader gefunden wurde
			if (pReader== null)
				throw new Exception(ERR_TYPE_NOT_SUPPORTED + paulaDocFeature.cType);
			if (this.logger!= null) this.logger.debug(MSG_STD+ " parsing paula document: " 
					+ pFile.getCanonicalPath()
					+" with specific paula reader: " 
					+ pReader.getReaderName() + " v" +pReader.getReaderVersion());
