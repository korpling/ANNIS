...
launch_daemon() {
	/bin/sh <<EOF
		...
		java -Dfile.encoding=UTF-8 -Dannis.home=\$ANNIS_HOME -Dannisservice.pid_file=\$pid_file -cp \$classpath de.deutschdiachrondigital.dddquery.helper.AnnisServiceRunner -d <&- &
		pid=\$!
		echo \$pid
EOF
}
