...
public class PathTranslator3 extends AbstractPathTranslator {
	...
	public void inAAbsolutePathType(AAbsolutePathType node) {
		path.addCondition(new IsNullCondition(target.getColumn("everything", "parent")));
	}
...
//		path.addCondition(Join.eq(context.getColumn("everything", "struct"), target.getColumn("everything", "struct")));
	}

	@Override
	public void inAChildAxis(AChildAxis node) {
		path.addCondition(Join.eq(context.getColumn("everything", "pre"), target.getColumn("everything", "parent")));
	}
	...
	public void inAParentAxis(AParentAxis node) {
		...
		// everything.pre = everything.pre (nicht struct.id = struct.id)
		path.addCondition(Join.eq(context.getColumn("everything", "pre"), helper.getColumn("everything", "pre")));
		path.addCondition(Join.eq(helper.getColumn("everything", "parent"), target.getColumn("everything", "pre")));
	}
	
	// FIXME echt kleiner, größer
	@Override
	public void inADescendantAxis(ADescendantAxis node) {
		path.addCondition(Join.le(context.getColumn("everything", "pre"), target.getColumn("everything", "pre")));
		path.addCondition(Join.le(target.getColumn("everything", "pre"), context.getColumn("everything", "post")));
	}
	...
	public void inAAncestorAxis(AAncestorAxis node) {
		...
		// everything.pre = everything.pre (nicht struct.id = struct.id)
		path.addCondition(Join.eq(context.getColumn("everything", "pre"), helper.getColumn("everything", "pre")));
		path.addCondition(Join.ge(helper.getColumn("everything", "pre"), target.getColumn("everything", "pre")));
		path.addCondition(Join.ge(target.getColumn("everything", "pre"), helper.getColumn("everything", "post")));
	}
	
	@Override
	public void inASiblingAxis(ASiblingAxis node) {
		path.addCondition(Join.eq(context.getColumn("everything", "parent"), target.getColumn("everything", "parent")));
	}
	
	@Override
	public void inAFollowingAxis(AFollowingAxis node) {
		path.addCondition(Join.eq(context.getColumn("everything", "text_ref"), target.getColumn("everything", "text_ref")));
		path.addCondition(Join.lt(context.getColumn("everything", "token_right"), target.getColumn("everything", "token_left")));
	}
	
	@Override
	public void inAPrecedingAxis(APrecedingAxis node) {
		path.addCondition(Join.eq(context.getColumn("everything", "text_ref"), target.getColumn("everything", "text_ref")));
		path.addCondition(Join.gt(context.getColumn("everything", "token_left"), target.getColumn("everything", "token_right")));
	}
	...
	public void inAContainedAxis(AContainedAxis node) {
		path.addCondition(Join.ge(context.getColumn("everything", "right"), target.getColumn("everything", "right")));
		path.addCondition(Join.le(context.getColumn("everything", "left"), target.getColumn("everything", "left")));
	}
	
	@Override
	public void inAContainingAxis(AContainingAxis node) {
		path.addCondition(Join.le(context.getColumn("everything", "right"), target.getColumn("everything", "right")));
		path.addCondition(Join.ge(context.getColumn("everything", "left"), target.getColumn("everything", "left")));
	}
	
	@Override
	public void inAPrefixAxis(APrefixAxis node) {
		path.addCondition(Join.eq(context.getColumn("everything", "text_ref"), target.getColumn("everything", "text_ref")));
		path.addCondition(Join.eq(context.getColumn("everything", "right"), target.getColumn("everything", "right")));
		path.addCondition(Join.le(context.getColumn("everything", "left"), target.getColumn("everything", "left")));
	}
	...
	public void inASuffixAxis(ASuffixAxis node) {
		path.addCondition(Join.eq(context.getColumn("everything", "text_ref"), target.getColumn("everything", "text_ref")));
		path.addCondition(Join.ge(context.getColumn("everything", "right"), target.getColumn("everything", "right")));
		path.addCondition(Join.eq(context.getColumn("everything", "left"), target.getColumn("everything", "left")));
	}
	...
	public void inAOverlappingAxis(AOverlappingAxis node) {
		...
		conjunction1.addCondition(Join.ge(context.getColumn("everything", "left"), target.getColumn("everything", "left")));
		conjunction1.addCondition(Join.le(context.getColumn("everything", "left"), target.getColumn("everything", "left")));
		Conjunction conjunction2 = new Conjunction();
		conjunction2.addCondition(Join.ge(context.getColumn("everything", "right"), target.getColumn("everything", "right")));
		conjunction2.addCondition(Join.le(context.getColumn("everything", "right"), target.getColumn("everything", "left")));
	...
	public void inAOverlappingPrecedingAxis(AOverlappingPrecedingAxis node) {
		path.addCondition(Join.eq(context.getColumn("everything", "text_ref"), target.getColumn("everything", "text_ref")));
		path.addCondition(Join.ge(context.getColumn("everything", "right"), target.getColumn("everything", "right")));
		path.addCondition(Join.le(context.getColumn("everything", "right"), target.getColumn("everything", "left")));
	}
	...
	public void inAImmediatelyFollowingAxis(AImmediatelyFollowingAxis node) {
		...
		path.addCondition(Join.eq(context.getColumn("everything", "token_right"), target.getColumn("everything", "token_left").toString() + " - 1"));
	}
...
//			path.addCondition(Join.eq(target.getColumn("everything", "name"), "'" + token(node.getName()) + "'"));
		}
	}
	...
	public void inAAttributeNodeTest(AAttributeNodeTest node) {
		if (node.getName() != null) {
			path.addCondition(Join.eq(target.getColumn("everything", "attribute"), "'" + token(node.getName()) + "'"));
		}
		textValue = TextValue.Attribute;
//		TARGET.USETABLE("everything");
//		TARGET.USETABLE("everything");
	}
	...
	private JoinField textValue(APathExpr pathExpr) {
		...
		// context node, FIXME das wird schiefgehen, weil jetzt alles everything verwendet
		if (steps.isEmpty()) {
			if (oldTextValue == TextValue.Attribute)
				return new Column(target, "everything", "value");
			else
				return new Column(target, "everything", "span");
		}
	...
	public void caseAOrExpr(AOrExpr node) {
		...
		condition.setLhs(target.getColumn("everything", "pre"));
