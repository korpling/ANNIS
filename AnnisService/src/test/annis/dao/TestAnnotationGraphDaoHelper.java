...
public class TestAnnotationGraphDaoHelper extends AnnisHomeTest {
	...
	public void createSqlQuery() {
		...
		String expected = "" +
			"SELECT DISTINCT\n" + 
			"\ttokens.key, edge.pre, edge.post, edge.parent, edge.level, edge.zshg AS component, edge.edge_type, edge.edge_name, edge.anno_namespace AS edge_anno_namespace, edge.anno_name AS edge_anno_name, edge.anno_value AS edge_anno_value, node.*\n" +
			"FROM\n" +
			TOKEN_RELATION + "\n" +
			"\tJOIN struct_annotation AS node ON (tokens.text_ref = node.text_ref AND (tokens.min <= node.left_token AND tokens.max >= node.right_token OR node.left_token <= tokens.min AND tokens.min <= node.right_token OR node.left_token <= tokens.max AND tokens.max <= node.right_token))\n" +
			"\tJOIN rank_annotations AS edge ON (edge.node_ref = node.id)\n" + 
			"ORDER BY tokens.key, edge.pre";
	...
	public void createSqlQueryText() {
		...
		assertThat(annotationGraphDaoHelper.createSqlQuery(TEXT_ID), is(expected));
	}
