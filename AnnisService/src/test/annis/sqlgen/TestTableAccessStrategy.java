...
public class TestTableAccessStrategy {
	...
	public void computeSourceTablesRootNode() {
		...
		expectTableCount(NODE_TABLE, 1);
		expectTableCount(EDGE_TABLE, 1);
		assertUsedTables();
	}
	
	// if rank and struct table are materialized, root node only uses struct table
	@Test
	public void computeSourceTablesRootNodeRankAliasedToStruct() {
		when(node23.isRoot()).thenReturn(true);
		tableAccessStrategy.addTableAlias(EDGE_TABLE, NODE_TABLE);
		expectTableCount(NODE_TABLE, 1);
		assertUsedTables();
	}
	...
	// if RANK_ANNOTATION_TABLE and RANK_TABLE are materialized, only use as many tables as there are annotations
	@Test
	public void computeSourceTablesEdgeAnnotationsAliasedToRank() {
		when(node23.getEdgeAnnotations()).thenReturn(annotations);
		tableAccessStrategy.addTableAlias(EDGE_ANNOTATION_TABLE, EDGE_TABLE);
		expectTableCount(NODE_TABLE, 1);
		expectTableCount(EDGE_TABLE, 3);
		assertUsedTables();
	}
	
	// if RANK_ANNOTATION_TABLE, RANK_TABLE and STRUCT_TABLE are materialized, only use as many tables as there are annotations
	@Test
	...
	// both node and edge annotations, root and rank table join
	@Test
	...
	public void onlyOneRankTable() {
		tableAccessStrategy.aliasedTable(EDGE_TABLE, 2);
	}
	...
	public void isMaterializedTrue() {
		tableAccessStrategy.addTableAlias(EDGE_TABLE, NODE_TABLE);
		assertThat(tableAccessStrategy.isMaterialized(EDGE_TABLE, NODE_TABLE), is(true));
	}
	
	// join of two tables is not materialized
	@Test
	public void isMaterializedFalse() {
		assertThat(tableAccessStrategy.isMaterialized(EDGE_TABLE, NODE_TABLE), is(false));
	}
