...
/**
 * MultiFeatAnalyzer analyzes paula files which are bound to dtd multifeat.dtd and 
 * classifies them to multifeat files.
 * @author Florian Zipser
 * @version 1.0
 *
 */
public class MultiFeatAnalyzer extends AbstractAnalyzer 
{
//	 ============================================== private Variablen ==============================================
	private static final String TOOLNAME= 	"MultiFeatAnalyzer";	//Name dieses Tools
	private static final String VERSION= 	"1.0";					//Version dieses Tools
	private static final boolean DEBUG=	false;					//Debug-Schalter
	
	private static final String anaType= "MultiFeatData";			//Name des Analyse Typs
	private static final String comment= "analyzed by: "+ TOOLNAME + " this document contains multifeature data. These datas are bundled features for one structure or token element.";			//Kommentar zu diesem Analyse Typ
	
	private static final String KW_ATT_VAL_MULTI= "multiFeat";			//Attributwert, der ein Dokument als Tok typisiert
	
	private static final String[] analyzableDTDs=	{"paula_multiFeat.dtd", "sfb632_multi.dtd"};			//DTD von der die zu analysierende Datei sein muss
	private static final String[] analyzablePAULATypes= {"multiFeat"};		//analysierbare PAULA-Typen
...
//	 ============================================== öffentliche Methoden ==============================================
	 ...
	 */
	public MultiFeatAnalyzer() throws Exception
	{
...
//	 ============================================== öffentliche Methoden ==============================================
	...
	public boolean canAnalyze(AnalyzeContainer aCon) throws Exception
	{
		...
		if ((aCon.getPAULAType()!= null) && (!aCon.getPAULAType().equalsIgnoreCase("")))
		{
			...
			{
				//wenn DTD = paula_multi.dtd oder DTD= sfb632_multi.dtd
					            ...
					            Attributes attributes) throws SAXException
	{
		try
		{
			//Element MULTIFEATLIST gefunden
			if (this.canAnalyzeValue(qName, TAG_MULTI_MULTIFEATLIST))
			{
				for (int i= 0; i < attributes.getLength(); i++)
				{
					//Attribut MULTIFEATLIST.TYPE gefunden
					if (this.canAnalyzeValue(attributes.getQName(i), ATT_MULTI_MULTIFEATLIST_TYPE))
					{
						//TYPE= MultiFeat
						if (attributes.getValue(i).equalsIgnoreCase(KW_ATT_VAL_MULTI))
						{
