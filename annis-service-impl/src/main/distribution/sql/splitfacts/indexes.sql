...
CREATE INDEX idx__facts_edge_name__:id
  ON facts_edge_:id
  USING btree
  (edge_name varchar_pattern_ops, component_id, node_ref);


CREATE INDEX idx__facts_edge_namespace__:id
  ON facts_edge_:id
  USING btree
  (edge_namespace varchar_pattern_ops, component_id, node_ref);


CREATE INDEX idx__facts_edge_type__:id
  ON facts_edge_:id
  USING btree
  (edge_type, component_id, node_ref);

CREATE INDEX idx__facts_level__:id
  ON facts_edge_:id
  USING btree
  ("level", component_id, node_ref);

CREATE INDEX idx__facts_edge_annotation__:id
  ON facts_edge_:id
  USING btree
  (edge_anno_ref, component_id, node_ref);
...
CREATE INDEX idx__facts_parent__:id
  ON facts_edge_:id
  USING btree
  (parent, component_id, node_ref);


CREATE INDEX idx__facts_post__:id
  ON facts_edge_:id
  USING btree
  (post, component_id, node_ref);


CREATE INDEX idx__facts_pre__:id
  ON facts_edge_:id
  USING btree
  (pre, component_id, node_ref);

CREATE INDEX idx__facts_root__:id
  ON facts_edge_:id
  USING btree
  (root, component_id, node_ref);
