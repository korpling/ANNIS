...
public class DocBrowserController implements Serializable {
        ...
        public void run() {
            ...
            // check if a visualization is already initiated
            {
                if (createVis) {
                    ...
                    input = createInput(corpus, doc, config, visualizer.isUsingRawText(), nodeAnnoFilter, ui);
            ...
            ui.access(() -> {
                ...
                if (createVis && input != null) {
                    ...
                    Component vis1 = visualizer.createComponent(input, null);
     ...
     *
     * @param corpus
     *            the name of the toplevel corpus
     * @param docName
     *            the name of the document
     * @param config
     *            the visualizer configuration
     * @param isUsingRawText
     *            indicates, whether the text from text table is taken, or if the
     *            salt project is traversed.
     * @param nodeAnnoFilter
     *            A list of node annotation names for filtering the nodes or null if
    ...
    public static VisualizerInput createInput(String corpus, String docName, Visualizer config, boolean isUsingRawText,
        ...
        if (isUsingRawText) {
            ...
            WebResource res = Helper.getAnnisWebResource(ui).path("query").path("graph").path(encodedToplevelCorpus)
