  ...
  public void writeOutput(Writer writer) {
    try {
      ...
      Document document = builder.parse(
                            new InputSource(
                                  new StringReader(this.paula)
                                )
                          );
      
      //Use XPath API to retrieve all token from document
      NodeList tokenNodeList = XPathAPI.selectNodeList(
                                          document, 
                                          ".//tok"
                                        );
      ...
      writer.append(
                    "<html>" + 
                    "  <head>" + 
                    "    <title>Output from ExampleVisualizer</title>" + 
                    "  </head>" +
                    "  <body>"
                   );
      ...
      for(int i=0; i<tokenNodeList.getLength();i++) {
        ...
        String tokenId = tokenNode.getAttributes()
                                  .getNamedItem("_id")
                                  .getNodeValue();
        String color = this.markableMap.containsKey(tokenId) ? 
                         this.markableMap.get(tokenId) : "";
        
        writer.append("<font color=\"" + color + "\">" + 
                      tokenNode.getTextContent() + "</font> "
                     );
      }
