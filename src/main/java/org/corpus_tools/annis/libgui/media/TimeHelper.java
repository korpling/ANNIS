...
/**
 * Helper class for getting time annotations on {@link SSpan} from the covered token.
 * 
...
public class TimeHelper {
  ...
  /**
   * Get start and end time from the overlapped {@link SToken}. The minimum start time and maximum
   * end times are choosen.
   * 
   * @param node The span to start from.
   * @return A double array, will have length of 0 if no time annotations where found, one element
   *         of only start elements where found, 2 elements if both start and end time are found.
   */
