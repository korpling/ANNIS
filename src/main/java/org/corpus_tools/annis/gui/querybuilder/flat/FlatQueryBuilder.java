...
public class FlatQueryBuilder extends Panel implements Button.ClickListener {
  ...
  private String getAQLQuery() {
    ...
    String fullQuery =
        (ql.toString() + edgeQuery.toString() + sentenceQuery.toString() + metaQuery.toString());
        ...
        for (String corpus : corpusSelection) {
          List<Annotation> allAnnotations = client.get()
              .uri(uriBuilder -> uriBuilder.path("/corpora/{corpus}/node-annotations")
                  .queryParam("list_values", true).queryParam("only_most_frequent_values", false)
                  .build(corpus))
  ...
  public Set<String> getAvailableMetaNames() {
    Set<String> result = new TreeSet<>();
    if (UI.getCurrent() instanceof CommonUI) {
      WebClient client = ((CommonUI) UI.getCurrent()).getWebClient();
      // get current corpus selection
      Collection<String> corpusSelection = cp.getState().getSelectedCorpora();
      try {
        for (String corpus : corpusSelection) {
          for (AnnoKey key : Helper.getMetaAnnotationNames(corpus, client).toIterable()) {
