...
import org.corpus_tools.annis.model.AnnisConstants;
 ...
 * grid. This is useful for getting references of texts where the normal token
...
public class TextColumnExporter extends BaseMatrixExporter { // NO_UCD (use default)
    private class IsDominatedByMatch implements GraphTraverseHandler {
        ...
        public boolean checkConstraint(GRAPH_TRAVERSE_TYPE traversalType, String traversalId, SRelation relation,
            ...
            return relation == null || relation instanceof SDominanceRelation || relation instanceof SSpanningRelation;
    ...
    // a helping structure to handle crossing edges, must be global over all query
    // results
    ...
    // indicate, whether filter numbers were set by user, global over all query
    // results
    ...
    // a helping structure to determine the right order of match nodes over all
    // records
    ...
    // contains a sequence of match numbers per speaker ordered according to their
    // occurrence in text, globally over all query results
     ...
     * @param matrixIsFilled
     *            a boolean, which indicates, whether the adjacency is filled or not
     * @param singleMatches
     *            a list of singleton match numbers
     * 
     * @return a list of globally ordered match numbers, if data alignable or an
     *         empty list otherwise
     */
    private List<Long> calculateOrderedMatchNumbersGlobally(int[][] adjacencyMatrix, boolean matrixIsFilled,
            Set<Long> singleMatches) {
     ...
     * org.corpus_tools.annis.gui.exporter.SaltBasedExporter). This method creates and fills an
     ...
     *
     * @param graph
     *            an org.corpus_tools.salt.common.SDocumentGraph representation of a
     *            record
     * @param args
     *            a map containing parameters like 'filter' or 'metakeys', set by
     *            user
     * @param recordNumber
     *            the number of record within the record set returned for the user
     *            query
     * @param nodeCount
     *            the count of distinct match numbers in the whole record set
    ...
    public void createAdjacencyMatrix(SDocumentGraph graph, Map<String, String> args, int recordNumber, int nodeCount)
        ...
        if (graph != null) {
            ...
            if (orderedToken != null) {
                ...
                // iterate first time over tokens to figure out which speaker has matches and to
                ...
                for (SToken token : orderedToken) {
                    ...
                    graph.traverse(root, GRAPH_TRAVERSE_TYPE.BOTTOM_UP_DEPTH_FIRST, TRAV_PREPROCESSING,
                            traverserSpeakerSearch);
                    ...
                    if (!dominatedMatchCodes.isEmpty()) {
                        ...
                        if (filterNumbersIsEmpty) {
                            ...
                            tokenToMatchNumber.put(counterGlobal,
                                    dominatedMatchCodes.get(dominatedMatchCodes.size() - 1));

                            // set filter number to the ordered list
                            if (!matchNumbersOrdered
                                    .contains(dominatedMatchCodes.get(dominatedMatchCodes.size() - 1))) {
    ...
    /**
     * @see org.corpus_tools.annis.gui.exporter.BaseMatrixExporter#getFileEnding()
     * 
    ...
    public String getFileEnding() {
         ...
         * use different defaults for importing the data depending on the locale of
         * Windows. Thus use the more general "txt" ending which will trigger a
    ...
    public String getHelpMessage() {
        return "The TextColumnExporter exports matches surrounded by the context as a csv file. "
                + "The columns will be separated by tab mark. <br/>" + "Parameters: <br/>"
                ...
                + "build a hierarchy by definition. " + "There are three node numbers 1, 2  and 3. "
     ...
     * {@link #calculateOrderedMatchNumbersGlobally}.
     */
    @Override
