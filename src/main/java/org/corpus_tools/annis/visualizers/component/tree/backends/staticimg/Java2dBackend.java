...
import org.corpus_tools.annis.visualizers.component.tree.GraphicsBackend;
...
public class Java2dBackend implements GraphicsBackend<AbstractImageGraphicsItem> {
  ...
  private double getRotationAngle(Point2D origin, Point2D target) {
    ...
    double x =
        Math.acos((origin.getX() - target.getX()) * Math.signum(origin.getX() - target.getX()) / l);
  ...
  @Override
  public AbstractImageGraphicsItem makeLabel(String label, Point2D pos, Font font, Color color,
      Alignment alignment, Shape shape) {
