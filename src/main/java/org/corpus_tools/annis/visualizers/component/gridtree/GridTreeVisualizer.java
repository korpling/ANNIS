 ...
 * the License.
 */
package org.corpus_tools.annis.visualizers.component.gridtree;
 ...
 *
 * Note that all layers represent the same annotation name at different hierarchical depths, marked
 * level:0,1,2,... etc. on the left
 *
 * Known Bug: the visualizer does not handle crossing edges. Equal annotation names which cover the
 * same range or a subset of nodes and have the same hierarchical depths are not rendered correctly
 * by grid_tree. https://github.com/korpling/ANNIS/issues/14
 *
 * <h2>Mappings:</h2>
 * <ul>
 * <li>Specify the name of the annotation to be visualized in the grid with <b>node_key:name</b>.
 * Note that all grid levels visualize the same annotation name at different hierarchical
 * depths.</li>
 * <li>Specify the name of the base row with the <b>tok_key:name</b>. This is useful, if you want to
 * use a specific annotation layer for instead of the always existing tok layer. E.g. the falko
 * corpora often contain the ZH (Zielhypothesen)-layers.</li>
 *
...
public class GridTreeVisualizer extends AbstractVisualizer { // NO_UCD (unused code)
  ...
  private static class GridTreePanel extends Panel {
    ...
    public GridTreePanel(VisualizerInput visInput, VisualizationToggle visToggle) {
      ...
      /**
       * This abstract representation is used by the AnnotationGrid for creating the html table at
       * the end. The should be sorted by the row height, which is represented as the a string value
       * of the row integer. The would only work up to 10 rows.
       */
      ...
      /**
       * Get a list of sorted token for retrieving the token index of the most left token and fetch
       * the token index of the first token, so we have can calculate the offset of each token
       * index. Also the token index of the last token is fetched.
       */
      ...
      // init the traversal
      GraphTraverseHandler traverse =
          new Traverse(startIdx, endIdx, getNodeKey(), input.getNamespace(), table, token2index);
      ...
      /**
       * Add the last row. For placing it to the bottom of the table, we need to get the string
       * representation of the last index.
       */
     ...
     *
     * @param baseRowIdx the index of the row from which the salt ids are extracted. Most of the
     *        time the index would be "tok".
     * @param table abstract representation of the table which is rendered by {@link AnnotationGrid}
     */
  ...
  private static class Traverse implements GraphTraverseHandler {
    ...
    /**
     * Tracks the depth of the traversal. Steps are counted, when the node containes the defined
     * annotation key. This value is later used for the row index in the {@link AnnotationGrid}.
     */
    int depth = 0;
     ...
     * @param nodeKey the annotation key. Only nodes which contain this key will be taken into
     *        account
    ...
    private Traverse(int startIdx, int endIdx, String nodeKey, String namespace,
        Map<String, ArrayList<Row>> table, Map<SToken, Integer> token2index) {
    ...
    @Override
    public void nodeLeft(GRAPH_TRAVERSE_TYPE g, String string, SNode currNode, SRelation edge,
        SNode fromNode, long l) {
      assert depth >= 0;
    ...
    @Override
    public void nodeReached(GRAPH_TRAVERSE_TYPE g, String string, SNode currNode, SRelation edge,
        SNode fromNode, long l) {
      ...
      if (anno != null && currNode != null && currNode.getGraph() instanceof SDocumentGraph) {
        ...
        Range<Integer> overlappedSpan =
            Helper.getLeftRightSpan(currNode, (SDocumentGraph) currNode.getGraph(), token2index);
