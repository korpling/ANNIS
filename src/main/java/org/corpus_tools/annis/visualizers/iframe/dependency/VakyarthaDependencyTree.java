 ...
 * the License.
 */
package org.corpus_tools.annis.visualizers.iframe.dependency;
...
import org.corpus_tools.annis.visualizers.iframe.WriterVisualizer;
...
/**
 * Provides a dependence visualization based on vakyartha visualization, which was developed by Kim
 * Gerdes.
 * 
 * Requires SVG enabled browser. <br />
 *
 * Originally this visualization was token based. Now Vakyartha extended to visualize dependence
 * between any nodes. This must be configured in the resolver_vis_map table. <br />
 * 
 * Mappings: <br />
 * If a annotation key is specified with <b>node_key:key</b>, all nodes which carry this annotation
 * are searched for pointing relations and instead of the token span the annotation value is used.
 *
...
public class VakyarthaDependencyTree extends WriterVisualizer { // NO_UCD (unused code)
  ...
  private static final org.slf4j.Logger log =
      LoggerFactory.getLogger(VakyarthaDependencyTree.class);

  /**
   * If this mapping is not set in the resolver_vis_map table this visualization is only based on
   * the token level.
   */
  ...
  /**
   * Provid the whole annotation ((String) &lt;namespace&gt;::&lt;key&gt;=&lt;value&gt;) value with
   * namespace and annotation key, which was defined in the mappings with the Key
  ...
  private String getText(SNode node, VisualizerInput input) {
    ...
    List<DataSourceSequence> sequences =
        sDocumentGraph.getOverlappedDataSourceSequence(node, SALT_TYPE.STEXT_OVERLAPPING_RELATION);
  ...
  public void printHTMLOutput(VisualizerInput input, Writer writer, TreeSet<SNode> selectedNodes) {
    ...
    try {
      ...
      if (input.getFont() != null) {
        ...
        println("<link href=\"" + input.getFont().getUrl()
      ...
      println("<script type=\"text/javascript\" src=\""
          + input.getResourcePath("vakyartha/jquery.js") + "\"></script>", writer);
      println("<script type=\"text/javascript\" src=\""
  ...
  /**
   * If the {@link VakyarthaDependencyTree#MAPPING_NODE_KEY} is set, then the value of this mapping
   * is used for selecting the SNode. If the mapping is not set, it falls back to the default
   * behaviour and only SToken are are selected.
   */
  ...
  public void writeOutput(VisualizerInput input, Writer writer) {
    ...
    Map<SToken, Integer> token2index =
        Helper.createToken2IndexMap(input.getSResult().getDocumentGraph(), null);
    TreeSet<SNode> selectedNodes = new TreeSet<>(new Comparator<SNode>() {
      @Override
      ...
      private int getIdx(SNode snode) {
        Range<Integer> leftRight =
            Helper.getLeftRightSpan(snode, input.getSResult().getDocumentGraph(), token2index);
