 ...
 * <p>
 * It is possible to use this visualization to view entire texts even if you do not have coreference
 * annotations.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * <b>Implementation notes</b>: This code relies heavily on HTML-tables and has some logic that is
 * difficult to understand. A GWT-based rewrite would be a good alternative.
 * </p>
 * 
...
public class CorefVisualizer extends WriterVisualizer { // NO_UCD (use default)
  ...
  HashMap<String, HashMap<Long, Integer>> referentOfToken; // the Long ist the Referend, the Integer

  // means: { 0=incoming P-Edge, 1=outgoing
  // P-Edge, 2=both(not used anymore)}
  List<String> visitedNodes;

  LinkedList<TComponenttype> componenttype; // used to save which Node (with outgoing "P"-Edge)

  // gelongs to which component
  ...
  private boolean connectionOf(String pre, String now, long currentComponent) {
    ...
    if (!pre.equals(now) && referentOfToken.get(pre) != null && referentOfToken.get(now) != null) {
      for (long l : referentOfToken.get(pre).keySet()) {
        if (referentList.get((int) l) != null
            && referentList.get((int) l).component == currentComponent
   ...
   */
  private List<String> getAllTokens(SStructuredNode n, String name, TComponenttype c, long cnr,
      String namespace) {
    ...
    if (referentOfToken.get(id) != null) {
      for (long l : referentOfToken.get(id).keySet()) {
        if (referentList.get((int) l) != null && referentList.get((int) l).component == component
            && referentList.get((int) l).annotations != null
            && referentList.get((int) l).annotations.size() > 0) {
          int num = referentOfToken.get(id).get(l);
          if (num == 0 || num == 2) {
            for (SerializableAnnotation an : referentList.get((int) l).annotations) {
              if (nri == 1) {
                incoming = ", &lt;b&gt;incoming Annotations&lt;/b&gt;: " + an.getName() + "="
                    + an.getValue();
                nri--;
              } else {
          ...
          if (num == 1 || num == 2) {
            for (SerializableAnnotation an : referentList.get((int) l).annotations) {
              if (nro == 1) {
                outgoing = ", &lt;b&gt;outgoing Annotations&lt;/b&gt;: " + an.getName() + "="
                    + an.getValue();
                nro--; // remove l+"- "+
              } else {
  ...
  private boolean includeEdge(SRelation<? extends SNode, ? extends SNode> e, String namespace) {
    if (e instanceof SPointingRelation) {
      SPointingRelation rel = (SPointingRelation) e;
      if (componentNameForRelation(rel) != null && rel.getSource() != null
          && rel.getTarget() != null && rel.getLayers() != null && !rel.getLayers().isEmpty()
          && namespace.equals(rel.getLayers().iterator().next().getName())) {
        return true;
      }
    }
        ...
        if (!finalpositions.isEmpty()) {
          for (Long currentPositionComponent : finalpositions) {
            ...
            if (currentPositionComponent.equals(Long.MIN_VALUE)) {
              ...
              if (lastId != null && currentId != null && checklist != null
                  && checklist.size() > index && checklist.get(index).booleanValue() == true) {
      ...
      if (outgoing != null) {
        for (SRelation<? extends SNode, ? extends SNode> e : outgoing) {
          if (!(e instanceof SPointingRelation) && e.getSource() instanceof SNode
              && e.getTarget() instanceof SNode) {
            List<String> Med = searchTokens(e.getTarget(), cnr);
            for (String s : Med) {
  ...
  public void writeOutput(VisualizerInput input, Writer w) {
    ...
    try {
      ...
      linkJQueryUI
          .setHref(input.getResourcePath("coref/jquery-ui-1.11.4.custom/jquery-ui.min.css"));
      ...
      linkJQueryUIStructure.setHref(
          input.getResourcePath("coref/jquery-ui-1.11.4.custom/jquery-ui.structure.min.css"));
      ...
      for (SRelation rawRel : edgeList) {
        if (includeEdge(rawRel, input.getNamespace())) {
          ...
          for (componentnr = 0; componentnr < componenttype.size(); componentnr++) {
            if (componenttype.get(componentnr) != null
                && componenttype.get(componentnr).type != null
      ...
      if (texts != null && !texts.isEmpty()) {
        ...
        for (STextualDS t : texts) {
          DataSourceSequence<Integer> sequence =
              new DataSourceSequence<>(t, 0, (t.getText() != null) ? t.getText().length() : 0);
          List<SToken> token =
              saltGraph.getSortedTokenByText(saltGraph.getTokensBySequence(sequence));
          ...
          if (token != null) {
            ...
            if (Boolean.parseBoolean(input.getMappings().getOrDefault("hide_empty", "false"))) {
              ...
              for (SToken tok : token) {
                /*
                 * The token is only added to this map if an valid edge (according to the resolver
                 * trigger) conntected to this token was found.
                 */
        ...
        Text errorTxt = new Text("Could not find any texts for the " + input.getNamespace()
            + " node namespace (layer).");
