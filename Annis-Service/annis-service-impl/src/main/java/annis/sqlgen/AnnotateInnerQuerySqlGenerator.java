...
public class AnnotateInnerQuerySqlGenerator 
	...
	implements SelectClauseSqlGenerator, OrderByClauseSqlGenerator, LimitOffsetClauseSqlGenerator {
	...
	public Object extractData(ResultSet rs) throws SQLException, DataAccessException {
		throw new NotImplementedException("BUG: inner query result is evaluated by outer query");
	}
	...
	public String selectClause(QueryData queryData, List<AnnisNode> alternative, String indent) {
		...
		for (int i = 1; i <= alternative.size(); ++i) {
			...
			fields.add("\n" + indent + TABSTOP + tables.aliasedColumn(NODE_TABLE, "id") + " AS id" + i);
	...
	@Override
	public String orderByClause(QueryData queryData, List<AnnisNode> alternative, String indent) {
	...
	@Override
	public String limitOffsetClause(QueryData queryData, List<AnnisNode> alternative, String indent) {
		AnnotateQueryData annotateQueryData = getAnnotateQueryData(queryData);
