...
import annis.sqlgen.BaseNodeSqlGenerator;
...
/**
 * QueryExecutor that is only able to handle a query on one single span value.
 * @author thomas
 */
public class SpanExecutor implements QueryExecutor
{
  ...
  public boolean checkIfApplicable(QueryData aql)
  {
    if(aql.getAlternatives().size() == 1
      ...
      && aql.getAlternatives().get(0).get(0).getSpannedText() != null)
    {
      return true;
    }
  ...
  public void createMatchView(JdbcTemplate jdbcTemplate, List<Long> corpusList, QueryData queryData)
  {
    ...
    AnnisNode n = queryData.getAlternatives().get(0).get(0);
