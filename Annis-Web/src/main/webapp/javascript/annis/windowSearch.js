...
var Citation = {
  generate : function()
  {
    ...
    var selections = Ext.ComponentMgr.get('gridPanelCorpus').selModel
        .getSelections();
  ...
  {
    // finally we evaluate the citation cookie value and set up the search
    // window accordingly
    var cookies = document.cookie;
    // grep citation value from cookie
    var citation = (cookies.indexOf("citation=") > -1) ? cookies.replace(
        /.*?citation="(.*?)".*/, "$1") : "";
    if ("" !== citation)
    {
      ...
      Ext.each(citation.split('),'), function(item)
      {
        ...
        if ("AQL" == key)
        {
          formPanelSearch.getComponent('queryAnnisQL').setValue(
              Url.decode(value.replace('"', '')));
        } else if ("CIDS" == key)
        {
...
Ext
    .onReady(function()
    {
      ...
      function updateStatus()
      {
        ...
        if (lastQuery === formPanelSearch.getComponent('queryAnnisQL')
            .getValue())
        {
        ...
        } else
        {
          ...
          Ext.Ajax.request({
            ...
            {
              formPanelSearch.getComponent('matchCount').setValue(
                  "FATAL ERROR: Unable to validate query.");
              lastStatus = formPanelSearch.getComponent('matchCount')
                  .getValue();
            },
            autoAbort : false,
      ...
      var panelQueryBuilder = new Ext.Panel(
          {
            ...
            region : 'center',
            items : {
              html : '<iframe id="iframeQueryBuilder" name="iframeQueryBuilder" src="'
                  + queryBuilderURL
