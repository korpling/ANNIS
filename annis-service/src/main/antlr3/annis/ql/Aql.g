...
dominance
	: REF DOMINANCE REF -> ^(PRECEDENCE  TOK ^(FROM_TO REF REF) ^(RANGE))
	| REF DOMINANCE STAR REF -> ^(PRECEDENCE TOK ^(FROM_TO REF REF) ^(RANGE STAR))
	| REF DOMINANCE min=DIGITS (COMMA max=DIGITS)? REF -> ^(PRECEDENCE TOK ^(FROM_TO REF REF) ^(RANGE $min $max?))
	| REF DOMINANCE layer=ID REF -> ^(PRECEDENCE  $layer ^(FROM_TO REF REF) ^(RANGE))
	| REF DOMINANCE layer=ID STAR REF -> ^(PRECEDENCE $layer ^(FROM_TO REF REF) ^(RANGE STAR))
	| REF DOMINANCE layer=ID COMMA? min=DIGITS (COMMA max=DIGITS)? REF -> ^(PRECEDENCE $layer ^(FROM_TO REF REF) ^(RANGE $min $max?))
	;
