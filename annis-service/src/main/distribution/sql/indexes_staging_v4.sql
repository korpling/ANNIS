CREATE UNIQUE INDEX tmpidx_pk_node ON _node (id);
CREATE UNIQUE INDEX tmpidx_pk_node_token ON _node(id) WHERE token_index is not null;
CREATE INDEX tmpidx_unique_name_appendix_helper ON _node ("name", corpus_ref, id); 

CREATE UNIQUE INDEX tmpidx_pk_corpus ON _corpus (id);

CREATE UNIQUE INDEX tmpidx_pk_component ON _component (id);
