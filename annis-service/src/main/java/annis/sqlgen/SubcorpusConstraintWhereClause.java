...
/**
 * Adds the constraint that all nodes are always inside one document as WHERE clause elements.
 *
 * @author Thomas Krause <krauseto@hu-berlin.de>
 */
public class SubcorpusConstraintWhereClause extends TableAccessStrategyFactory
    implements WhereClauseSqlGenerator<QueryData> {

  @Override
  public Set<String> whereConditions(QueryData queryData, List<QueryNode> alternative,
      String indent) {
  ...
  // VR: inline
  @Deprecated
  public List<String> commonWhereConditions(List<QueryNode> nodes, List<Long> corpusList,
      List<Long> documents) {
    ...
    for (int left = 0; left < copyNodes.length; left++) {
      for (int right = 0; right < copyNodes.length; right++) {
        if (left != right) {
          // only add constraint if the two nodes are not already connected by their component or
          // node id
          ...
          if (needsCorpusRef) {
            conditions
                .add(join("=", tables(copyNodes[left]).aliasedColumn(NODE_TABLE, "corpus_ref"),
