  ...
  public void caseAAndExpr(AAndExpr node)
  {
    ...
    for (PExpr expr : node.getExpr())
    {
      if (expr instanceof ATextSearchExpr
        ...
        || expr instanceof AAnnotationSearchExpr
        || expr instanceof AAnyNodeSearchExpr)
      {
    ...
    if (connectedNodeClasses.size() > 1)
    {
      ...
      {
        throw new AnnisQLSemanticsException(
          "Variable bound to non-existing node.");
      }
      else
      {
    ...
    boolean connected = expected.size() == actual.size() && actual.containsAll(
      expected);
