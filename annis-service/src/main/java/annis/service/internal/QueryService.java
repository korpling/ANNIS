...
public class QueryService {
    ...
    public Response find(@QueryParam("q") String query,
        ...
        if ("text/plain".equals(bestMediaTypeMatch)) {
            ...
            return Response.ok().type("application/xml")
                    .entity(new GenericEntity<MatchGroup>(new MatchGroup(result)) {}).build();
        }
     ...
     *
     * @throws WebApplicationException
     *             Thrown if some corpora are unknown to the system.
     */
     ...
     * 
     * @param rawFields
     *            Comma seperated list of result vector elements. Each element has
     *            the form <node-nr>:<anno-name>. The annotation name can be set to
     *            "tok" to indicate that the span should be used instead of an
     *            annotation.
     * @throws GraphANNISException
     */
    @GET
     ...
     *
     * @param context
     *            Must be a valid key of the corpus configuration section in the
     *            annis-service.properties file.
     ...
     *
     * @param rawCorpusNames
     *            specifies the corpora the examples are fetched from.
     *
     */
    @GET
