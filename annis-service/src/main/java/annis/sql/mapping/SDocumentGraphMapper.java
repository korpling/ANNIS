...
/**
 * Maps the SQL to a document graph.
 * @author Thomas Krause <krauseto@hu-berlin.de>
 */
public class SDocumentGraphMapper
{

  private final static Logger log = LoggerFactory.getLogger(SDocumentGraphMapper.class);

  private final org.eclipse.emf.common.util.URI documentURI;
  
  public SDocumentGraphMapper(org.eclipse.emf.common.util.URI documentURI)
  {
    this.documentURI = documentURI;
  }

  /**
   * Maps a single document graph from a result set.
   * 
   * @param resultSet
   * @return
   * @throws SQLException
      ...
      if (!resultSet.wasNull())
      {
        ...
        if (node instanceof SSpan)
        {
          componentForSpan.put(node.getId(), new ComponentEntry(componentID,
            'c',
            resultSet.getString("component_layer"),
    ...
    FastInverseMap<Long, SNode> nodeByRankID) throws SQLException
  {
    ...
    String saltID = resultSet.getString("node_salt_id");
    if (saltID == null)
    {
    ...
    String relationType = resultSet.getString("component_type");

    long tokenIndex = resultSet.getLong("node_token_index");
    boolean isToken = !resultSet.wasNull();

    org.eclipse.emf.common.util.URI nodeURI =  documentURI.appendFragment(saltID);
    ...
    if (node == null)
    {
      ...
      long textRef = resultSet.getLong("node_text_ref");
    ...
    String nodeAnnoValue = resultSet.getString("node_annotation_value");
    String nodeAnnoNameSpace = resultSet.getString("node_annotation_namespace");
    String nodeAnnoName = resultSet.getString("node_annotation_name");
    if (!resultSet.wasNull())
    {
    ...
    String segName = resultSet.getString("node_seg_name");
    if (segName != null)
    {
      long left = resultSet.getLong("node_seg_index");
    ...
    TreeMap<Long, SToken> tokenByIndex) throws SQLException
  {
    ...
    tokenTexts.put(tokenIndex, resultSet.getString("node_span"));
  ...
  private void mapLayer(SStructuredNode node, SDocumentGraph graph,
    ...
    throws SQLException
  {
    String namespace = resultSet.getString("node_layer");
  ...
  private void addRelationAnnotations(ResultSet resultSet, SRelation rel)
    throws SQLException
  {
    String relationAnnoValue = resultSet.getString("edge_annotation_value");
    String relationAnnoNameSpace = resultSet.getString("edge_annotation_namespace");
    String relationAnnoName = resultSet.getString("edge_annotation_name");
    if (!resultSet.wasNull())
    {
  ...
  private void createRelation(ResultSet resultSet, SDocumentGraph graph,
    ...
    SQLException
  {
    long parent = resultSet.getLong("rank_parent");
    if (resultSet.wasNull())
    {
      return;
    }

    long pre = resultSet.getLong("rank_pre");
    long componentID = resultSet.getLong("component_id");
    String relationNamespace = resultSet.getString("component_layer");
    if (relationNamespace == null)
    {
      relationNamespace = "default_ns";
    }
    String relationName = resultSet.getString("component_name");
