 ...
 * 
 * Adjusted to Apache Commons Lang3 by Thomas Krause
 * {@literal <krauseto@hu-berlin.de>}
 * 
 */
public final class MIMEParse {
    ...
    /**
     * Find the best match for a given mimeType against a list of media_ranges that
     * have already been parsed by MimeParse.parseMediaRange(). Returns a tuple of
     ...
     * ('application', '*', {'q', '0.5'})
     * 
     * In addition this function also guarantees that there is a value for 'q' in
     * the params dictionary, filling it in with a proper default if necessary.
     * 
     * @param range
     */
    ...
    /**
     * Returns the quality 'q' of a mime-type when compared against the mediaRanges
    ...
    /**
     * Find the best match for a given mime-type against a list of ranges that have
     * already been parsed by parseMediaRange(). Returns the 'q' quality parameter
     * of the best match, 0 if no match was found. This function bahaves the same as
     * quality() except that 'parsed_ranges' must be a list of parsed media ranges.
     * 
     * @param mimeType
     * @param parsedRanges
     * @return
